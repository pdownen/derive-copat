{-# LANGUAGE LambdaCase #-}
module Copattern.Nest.Env.CPS.Double where
...
run r = (response r []) Final

eval :: (Eq i, Eq a) => Term i a -> Answer i a
eval m = (term m []) Nop Final

try :: (Eq i, Eq a) => Option i a -> Answer i a
try o = (option o []) Nop (term Raise []) Nop Final
