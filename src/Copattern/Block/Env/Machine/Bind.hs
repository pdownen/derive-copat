{-# LANGUAGE LambdaCase #-}
module Copattern.Block.Env.Machine.Bind where
...
comatch :: (Eq a, Eq i)
        ...
        -> CPSCopat i a
comatch Nop        rhs env = \_ _   -> (refocus rhs env)
comatch (x :* lhs) rhs env = \q ops -> \case
  (y :* k) -> (comatch lhs rhs ((x, useArg y) : env)) q ops k
  Nop      -> Under $ (comatch (x :* lhs) rhs env) q ops
  _        -> ops q
comatch (i :@ lhs) rhs env = \q ops -> \case
  (j :@ k) | i == j -> (comatch lhs rhs env) q ops k
  Nop               -> Under $ (comatch (i :@ lhs) rhs env) q ops
  _                 -> ops q
