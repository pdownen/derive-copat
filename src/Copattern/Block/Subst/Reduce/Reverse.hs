{-# LANGUAGE LambdaCase #-}
module Copattern.Block.Subst.Reduce.Reverse where
...
found (ComatchCxt k (rhs, ops, q)) = \case
  Comatch env (Followup q')   -> Next (Reduced $ rhs // k ++ env) q'
  Comatch env (Unasked  lhs') -> More  lhs' (rhs // k ++ env) ops q
