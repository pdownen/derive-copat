{-# LANGUAGE LambdaCase #-}
module Copattern.Block.Subst.Reduce.Defunc where
...
reduce (Respond (lhs :-> rhs : ops)) q = comatch lhs q $ OnMatch rhs ops q
...
data CoMatchCxt i a
  = OnMatch (Term i a) [Option i a] (Question i a)
  | BindArg (a, Term i a) (CoMatchCxt i a)
